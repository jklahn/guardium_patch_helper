...
import os
from sdk_gpylib import SdkGpylib
from live_gpylib import LiveGpylib

# ===== SDK specifics =====

def is_sdk():
    sdk_env = os.getenv('GUARD_APPFW_SDK', 'no').lower() == 'true'
    return sdk_env

def strategy():
    strategy_impl = LiveGpylib()
    if is_sdk():
        strategy_impl = SdkGpylib()
...
def get_CSRF_token():
    return strategy().get_CSRF_token()

def get_app_base_url():
    return strategy().get_app_base_url()

def g_url_for(endpoint, **values):
    return strategy().g_url_for(endpoint, **values)
